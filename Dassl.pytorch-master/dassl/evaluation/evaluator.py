...
# 指定包含类别名字的 .txt 文件的路径
file_path = "/home/lzy/bisai/baseline/datas/classes.txt"
...
with open(file_path, "r") as file:
    for line in file:
        category = line.strip()  # 去除每行两端的空白字符，例如换行符
...
def mAP(targs, preds, merged=False):
...
class MLClassification(EvaluatorBase):
    ...
    def process(self, mo, gt, mo_aux=None, mo_aux2=None, mo_aux3=None, mo_aux4=None, mo_aux5=None, mo_aux6=None, mo_aux7=None):
        ...
        if mo_aux2 is not None:
            self._y_pred_aux2.append(mo_aux2.data)
        if mo_aux3 is not None:
            self._y_pred_aux3.append(mo_aux3.data)
        if mo_aux4 is not None:
            self._y_pred_aux4.append(mo_aux4.data)
        if mo_aux5 is not None:
            self._y_pred_aux5.append(mo_aux5.data)
        if mo_aux6 is not None:
            self._y_pred_aux6.append(mo_aux6.data)
        if mo_aux7 is not None:
            self._y_pred_aux7.append(mo_aux7.data)
    ...
    def evaluate(self):
        ...
        if len(self._y_pred_aux2) > 0:
            preds_aux2 = torch.cat(self._y_pred_aux2, dim=0)
        else:
            preds_aux2 = None
            
        if len(self._y_pred_aux3) > 0:
            preds_aux3 = torch.cat(self._y_pred_aux3, dim=0)
        else:
            preds_aux3 = None
        
        if len(self._y_pred_aux4) > 0:
            preds_aux4 = torch.cat(self._y_pred_aux4, dim=0)
        else:
            preds_aux4 = None 
            
        if len(self._y_pred_aux5) > 0:
            preds_aux5 = torch.cat(self._y_pred_aux5, dim=0)
        else:
            preds_aux5 = None
        
        if len(self._y_pred_aux6) > 0:
            preds_aux6 = torch.cat(self._y_pred_aux6, dim=0)
        else:
            preds_aux6 = None 
            
        if len(self._y_pred_aux7) > 0:
            preds_aux7 = torch.cat(self._y_pred_aux7, dim=0)
        else:
            preds_aux7 = None     
            
#         tmp = self.cfg.TRAINER.Caption.GL_merge_rate
#         preds_merge = preds.cpu().numpy() * 0.2 + preds_aux.cpu().numpy() * 0.2 \
#                 + preds_aux2.cpu().numpy() * 0.1 + preds_aux3.cpu().numpy() * 0.1 \
#                 + preds_aux4.cpu().numpy() * 0.2 + preds_aux5.cpu().numpy() * 0.2
#         mAP_score = 0
...
#             json.dump(ret_list, f)
        ...
        if self.activate_func == 'default_merge_aux':
            ...
            if len(self._y_pred_aux2) > 0:
                mAP_score_aux2 = mAP(targets, preds_aux2.cpu().numpy())
                print("mAP score aux2:", mAP_score_aux2)
            if len(self._y_pred_aux3) > 0:
                mAP_score_aux3 = mAP(targets, preds_aux3.cpu().numpy())
                print("mAP score aux3:", mAP_score_aux3)
            if len(self._y_pred_aux4) > 0:
                mAP_score_aux4 = mAP(targets, preds_aux4.cpu().numpy())
                print("mAP score aux4:", mAP_score_aux4) 
            if len(self._y_pred_aux5) > 0:
                mAP_score_aux5 = mAP(targets, preds_aux5.cpu().numpy())
                print("mAP score aux5:", mAP_score_aux5) 
            if len(self._y_pred_aux6) > 0:
                mAP_score_aux6 = mAP(targets, preds_aux6.cpu().numpy())
                print("mAP score aux6:", mAP_score_aux6)
            if len(self._y_pred_aux7) > 0:
                mAP_score_aux7 = mAP(targets, preds_aux7.cpu().numpy())
                print("mAP score aux7:", mAP_score_aux7)
                
            tmp = self.cfg.TRAINER.Caption.GL_merge_rate
            if len(self._y_pred_aux) > 0 and len(self._y_pred_aux2) > 0 and len(self._y_pred_aux3) > 0 and len(self._y_pred_aux4) > 0 \
                and len(self._y_pred_aux5) > 0:
                preds_merge = preds.cpu().numpy() * 0.2 + preds_aux.cpu().numpy() * 0.2 \
                        + preds_aux2.cpu().numpy() * 0.1 + preds_aux3.cpu().numpy() * 0.1 \
                        + preds_aux4.cpu().numpy() * 0.2 + preds_aux5.cpu().numpy() * 0.2 
                
                mAP_score_ = mAP(targets, preds_merge, merged=True)
                print("mAP score merged : preds + preds_aux + preds_aux2 + preds_aux3 + preds_aux4 + preds_aux5", mAP_score_)
...
#                     json.dump(ret_list, f)
#                 pdb.set_trace()
#                 with open('6608.pkl', 'wb') as f:
#                     pickle.dump((targets, preds, preds_aux,preds_aux2,preds_aux3,preds_aux4,preds_aux5), f)
                        
                # only for the original clip prompts
                if len(self._y_pred_aux6) > 0 and len(self._y_pred_aux7) > 0:
                    preds_merge2 = preds_aux6.cpu().numpy() * 0.5 + preds_aux7.cpu().numpy() * 0.5
                    mAP_score_2 = mAP(targets, preds_merge2)
                    print("mAP score ori prompts: preds + preds_aux ", mAP_score_2)
                    
                    final_preds_merge = preds_merge + preds_merge2 * 0.1
                    mAP_score_final = mAP(targets, final_preds_merge)
                    print("mAP score ori prompts and our methods: ", mAP_score_final)
                    
                mAP_score = mAP_score_
                
            elif len(self._y_pred_aux) > 0 and len(self._y_pred_aux2) > 0 and len(self._y_pred_aux3) > 0:
                
                preds_merge = preds.cpu().numpy() * 0.25 + preds_aux.cpu().numpy() * 0.25 \
                        + preds_aux2.cpu().numpy() * 0.25 + preds_aux3.cpu().numpy() * 0.25
                    
                mAP_score_ = mAP(targets, preds_merge)
                print("mAP score merged : preds + preds_aux + preds_aux2 + preds_aux3 :", mAP_score_)
                mAP_score = mAP_score_

            elif len(self._y_pred_aux) > 0 and len(self._y_pred_aux2):
                preds_merge = preds.cpu().numpy() * 0.4 + preds_aux.cpu().numpy() * 0.4 + preds_aux2.cpu().numpy() * 0.2
                mAP_score_ = mAP(targets, preds_merge)
                print("mAP score merged : preds + preds_aux + preds_aux2:", mAP_score_)
                mAP_score = mAP_score_
                
            elif len(self._y_pred_aux) > 0:
